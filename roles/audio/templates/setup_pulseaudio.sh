...
TRX_SPKR="pci-0000_00"
TRX_MIC="pci-0000_00"

HEADSET_MIC_NR=$(pactl list short sources| grep $HEADSET_MIC | grep -v monitor | sed -e "s/[[:space:]]\+/ /g" | cut -d ' ' -f1)
HEADSET_SPKR_NR=$(pactl list short sinks| grep $HEADSET_SPKR | grep -v monitor | sed -e "s/[[:space:]]\+/ /g" | cut -d ' ' -f1)

TRX_SPKR_NR=$(pactl list short sources| grep $TRX_SPKR | grep -v monitor | sed -e "s/[[:space:]]\+/ /g" | cut -d ' ' -f1)
TRX_MIC_NR=$(pactl list short sinks| grep $TRX_MIC | grep -v monitor | sed -e "s/[[:space:]]\+/ /g" | cut -d ' ' -f1)
